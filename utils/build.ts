...
const gitChangesArg = process.argv.find(arg => arg.startsWith('--git-changes='))
...
function determineVersionIncrement(changes: TokenChange[]): 'major' | 'minor' | 'patch' | null {
  ...
  for (const change of changes) {
    if (change.type === 'deleted') {
      ...
      const beforeMap = new Map(before.map(token => [`${token.address}-${token.chainId}`, token]))
      const afterMap = new Map(after.map(token => [`${token.address}-${token.chainId}`, token]))
...
glob("chains/*.json", {}, async function (er, files) {
  ...
  if (shouldIncrementVersion) {
    ...
    if (incrementType === null) {
      process.stdout.write('No semantic changes detected in token files - skipping version increment\n')
      ...
      case 'major':
        ...
        process.stdout.write(`Major version increment (tokens removed/addresses changed): ${tokenlistTemplate.version.major}.${tokenlistTemplate.version.minor}.${tokenlistTemplate.version.patch} -> ${newVersion.major}.${newVersion.minor}.${newVersion.patch}\n`)
        break
      case 'minor':
        newVersion = {
        ...
        process.stdout.write(`Minor version increment (tokens added): ${tokenlistTemplate.version.major}.${tokenlistTemplate.version.minor}.${tokenlistTemplate.version.patch} -> ${newVersion.major}.${newVersion.minor}.${newVersion.patch}\n`)
        break
      case 'patch':
        newVersion = {
          ...tokenlistTemplate.version,
        ...
        process.stdout.write(`Patch version increment (token details changed): ${tokenlistTemplate.version.major}.${tokenlistTemplate.version.minor}.${tokenlistTemplate.version.patch} -> ${newVersion.major}.${newVersion.minor}.${newVersion.patch}\n`)
        break
      }
